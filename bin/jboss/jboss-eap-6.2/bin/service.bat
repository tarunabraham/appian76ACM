...
REM
if "x%SVCNAME%" == "x" (
	set SVCNAME=JBOSSAS7SVC
)
set SVCDISP=JBoss EAP 6.1
set SVCDESC=JBoss EAP 6.1Final/Platform: Windows %PROCESSOR_ARCHITECTURE%
set NOPAUSE=Y
...
echo Y > .r.lock
jbosssvc.exe -p 1 "Starting %SVCDISP%" > startup.log
call standalone.bat < .r.lock >> startup.log 2>&1
jbosssvc.exe -p 1 "Shutdown %SVCDISP% service" >> startup.log
del .r.lock
goto cmdEnd
...
echo Y > .r.lock
jbosssvc.exe -p 1 "Restarting %SVCDISP%" >> startup.log
call standalone.bat < .r.lock >> startup.log 2>&1
jbosssvc.exe -p 1 "Shutdown %SVCDISP% service" >> startup.log
del .r.lock
goto cmdEnd
